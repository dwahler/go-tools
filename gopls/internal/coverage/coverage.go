...
import (
	...
	"github.com/dwahler/go-tools/cover"
)
...
func main() {
	...
	parg := "github.com/dwahler/go-tools/internal/lsp/..."
...
func checkCwd() {
	...
	// we expect to be a the root of github.com/dwahler/go-tools
	cmd := exec.Command("go", "list", "-m", "-f", "{{.Dir}}", "github.com/dwahler/go-tools")
	buf, err := cmd.Output()
	buf = bytes.Trim(buf, "\n \t") // remove \n at end
	if err != nil {
		log.Fatal(err)
